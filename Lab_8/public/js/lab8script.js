...
async function getData() {
    await fetch(url, settings)
        ...
        .then((json) => {
            ...
            for (x = 0; x < 5; x++) {
                const random = getRandomInt(0,listSize);
                console.log(random)
            const reddit = json.data.children[random].data;
            console.log(reddit)
            const subreddit = reddit.subreddit
            const author = reddit.author
            const title = reddit.title
            const ups = reddit.ups
const mylist = document.getElementById("redditList")
